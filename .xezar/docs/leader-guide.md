...
- Base branch: read `baseBranch` from `.xezar/config.json`; task pull requests target it. The default branch takes releases only.
- Gate, one command: `.xezar/checks/repo-gates.sh`. It runs `npm ci`, the security scan, `npm run typecheck`, `npm run lint`, `npm run format:check`, `npm test`, `npm run build`, `npm run check:headers`, then the repository checks.
- Required CI jobs: `check`, `e2e`, `e2e-macos`. The Docker e2e suite runs in CI only.
- Source: `bin/`, `src/`. Tests: `test/`. Docs: `README.md`, `CLAUDE.md`, `.xezar/docs/`. Design gate: **on**.

## Task lifecycle stages

triage → plan/spec → design (`needs-design` → `design` → `design-approved` | `design-failed`) → implementation (`in-progress`) → gates → `review` ↔ `changes-requested` → QA (`needs-qa` → `qa` → `qa-approved` | `qa-failed`) → `merge-queue` → merged. `blocked` and `do-not-merge` stop any stage.

## Routing, accounts and limits

- The table is `.xezar/docs/model-routing.md`: one ordered chain per row, ending in `wait`.
- The reserved leader login is this machine's default Claude Code login. **It runs no tasks.** Its id, and every lane's account, is in `.local/xezar/runtime/onboarding-identity.json`. A missing task login is a hard stop, never a fallback to the leader login.
- The leader runs on Fable 5.x or Opus 5.x and on nothing weaker. There is no second leader login: when the leader's limit ends, finish in-flight work, commit the record, stop.
- A lane that is out is recorded per `.xezar/docs/account-limits.md` with its reset time; it expires to `unknown`. Unlimited lanes (local models, pay-per-use API) are not tracked.

## Release runbook

The owner authorises a release, quoting the commit. Steps: bump version, `CHANGELOG.md` and `CITATION.cff` on the base branch → pull request base → default branch, CI green → signed tag → publish a GitHub Release → `.github/workflows/publish.yml` publishes to npm (OIDC, protected `release` environment). Proof: the merged pull request, the tag, the workflow run, the npm version.
