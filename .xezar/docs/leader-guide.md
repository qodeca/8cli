...
- Base branch: `develop` – every task pull request targets it. `main` is the release branch, reached only by a release.
- Source: `bin/` and `src/` (TypeScript, every file carries the SPDX header). Tests: `test/` (unit), `test/e2e/` (Docker), `test/e2e-macos/` (keychain).
- Documents: under `docs/`, by `paths.*` key in `.xezar/pipeline/config.json` (index: `docs/README.md`).
- Gate commands, in order: `npm run typecheck`, `npm run lint`, `npm run format:check`, `npm test`, `npm run build`, `npm run check:headers`.
- Run the whole gate with one command: `.xezar/checks/repo-gates.sh`.
- CI checks required on `develop`: `check`, `e2e`, `e2e-macos`. The e2e suites and `reuse lint` run only in CI.

## Task lifecycle stages

1. Triage – an issue gets a category, a priority and a risk label.
2. Design (when `needs-design` is set) – waits for `design-approved`, or `skip-design` with a reason.
3. Author – a workflow implements in a worktree and runs the gate; the draft PR opens with `review`.
4. Review – an independent lane reviews; `changes-requested` sends it back to the author.
5. QA – `needs-qa` until `qa-approved` (or `qa-self-verified` under the documented exception); `skip-qa` needs a reason.
6. Merge – `merge-queue`, then a squash merge once every required check is green.
`in-progress` marks a claim, `ci-monitoring` a CI watch; `blocked` and `do-not-merge` stop the line.

## Routing, accounts and limits

- Routing is `.xezar/routing.json`, read only through `.xezar/checks/route.mjs` (`.xezar/docs/routing.md`). Never pick a lane by hand.
- The reserved leader login is Claude Code's built-in login `default`. It runs no tasks and is in no rotation.
- Task logins are the rotations in `.xezar/routing.json`; the engine default for an unnamed task is the first Claude rotation login.
- The budget table lives in the live campaign's `README.md`, keyed runner × login (`.xezar/docs/account-limits.md`).
- A login that runs out is recorded there with its reset time; unknown is never "out" and never "fine".
- When the leader's own login runs out: finish what is in flight, commit the campaign record, and stop.

## Release runbook

- Only the owner authorises a release, in their own words recorded in `decisions.md`.
1. `xez-release` prepares it from a commit already on `develop`: changelog folded, version written. Proof: the release PR, green.
2. `develop` is merged into `main` by pull request. Proof: `check`, `e2e`, `e2e-macos` green on `main`.
3. A signed tag on `main`, then a GitHub Release. Proof: the tag verifies.
4. Publishing the Release runs `.github/workflows/publish.yml` (npm, OIDC, protected `release` environment). A person approves that environment; the leader never publishes. Proof: the package version on npm.
