...
- Base branch: `develop`. Releases are cut from `main`; work never lands on `main` directly.
- Gate, one command: `.xezar/checks/repo-gates.sh`
- The gate runs, in order: `npm ci` · `.xezar/checks/security-scan.sh` · `npm run typecheck` ·
  `npm run lint` · `npm run format:check` · `npm run check:headers` · `npm test` ·
  `npm run build` · `.xezar/checks/repository-checks.sh`
- Source: `src/` (clients, commands, formatters, keychain) and `bin/8cli.ts`.
- Tests: `test/` — unit via Vitest; `test/e2e/` needs Docker; `test/e2e-macos/` needs a real keychain.
  Neither e2e suite is in the gate: they are required CI checks instead.
- Docs: `README.md`, `CLAUDE.md`, `SDLC.md`, `CODE_REVIEW.md`, `BACKWARD_COMPATIBILITY.md`.
- Every `.ts` file under `bin/` and `src/` carries the SPDX header; `check:headers` enforces it.

## Task lifecycle stages

1. **Filed** — an issue exists with a category, a priority and a risk label.
2. **Ready** — it meets the Definition of Ready in `SDLC.md`. A feature needs a covering spec.
3. **In progress** — `in-progress` label, assignee set, work in its own worktree.
4. **Design** — only when the change alters a user-visible CLI surface: `needs-design`, then
   `design-approved` or `design-failed`.
5. **Review** — `review` label on the PR; a verdict gives `changes-requested` or moves on.
6. **QA** — `needs-qa`, then `qa-approved`, `qa-self-verified` or `qa-failed`. `skip-qa` is a
   deliberate, stated choice.
7. **Merge** — `merge-queue`, required checks green, squash-merge into `develop`.
8. **Release** — the owner gives the go, quoting the commit; see the runbook below.
`blocked` and `do-not-merge` override every stage above.

## Routing, accounts and limits

- The full table: `.xezar/docs/model-routing.md`. Read it before any dispatch.
- **The reserved leader login runs no tasks.** It is the harness's own built-in Claude login, and
  it is never a dispatch target and never the fallback when a chain runs out. It exists so the
  leader can still finish, write the record and stop when every lane is busy.
- A lane is a tool plus a model, not an account. Accounts are the rotation **underneath** a lane:
  when one runs out of tokens the next account for that lane takes over, and the lane is only
  "out" when every account for it is out.
- Every chain ends in `wait`. `wait` is real: the work waits. Never invent a fallback.
- A lane being out is recorded in the budget record with the reset time it reported. An entry that
  expires becomes `unknown`, which is neither "out" nor "fine" — the next real dispatch finds out.
  Never probe in a loop: a probe is a first use and opens a fresh window.
- The locally hosted models may read and advise. They never write to a branch.
- One model never reviews its own work, and a high-risk change needs a different account **and** a
  different vendor from the author.

## Release runbook

- **The owner authorises every release, quoting the commit.** The leader never starts one.
- The commit must already be merged and already an ancestor of `main`.
1. Confirm the commit is on `main` — `git merge-base --is-ancestor <commit> origin/main`.
2. Run the gate on that commit; all required checks green on `main`.
3. Fold the changelog fragments and write the version (`release-prep.yaml`).
4. Tag the commit, signed, and push the tag.
5. Publish the GitHub Release. That triggers `.github/workflows/publish.yml`, which publishes to
   npm through OIDC trusted publishing, behind the protected `release` environment.
- **The last step is always a person's.** The leader holds no publishing credential.
