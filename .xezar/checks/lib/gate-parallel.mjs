...
// phase is gates 3–8 — typecheck, lint, format:check, npm test, build, check:headers — and the lanes
// below encode AGENTS.md § Validation's schedule: `typecheck → build` in one lane, with lint,
// format:check, `npm test` and check:headers beside it. Gate 1 is the install and gate 2 is the security
// stage, both serial and both before this phase; gate 9 is the repository-check tail.
// Renumbering the canonical list means renumbering here AND in `gate-parallel.test.mjs`.
import { spawn } from 'node:child_process';
import { mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';

const APPLICATION_GATES = [3, 4, 5, 6, 7, 8];
const APPLICATION_LANES = [[3, 7], [4], [5], [6], [8]];
