...
# The fragment flow needs BOTH the changelog and a `changelog.d/` folder. This project keeps a
# Keep a Changelog file edited under `## [Unreleased]` and has no fragment folder, so the check is
# skipped, loudly, until the owner adopts fragments (create `changelog.d/` to turn it on).
